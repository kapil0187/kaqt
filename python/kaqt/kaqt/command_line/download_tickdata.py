...
from kaqt.foundation.constants import tickdata_remote_dir
...
def main():
    ...
    if missing_args:
        ...
        if kaqt_path() == '':
            ...
            return
        save_dir = options.dir if options.dir else os.path.join(kaqt_path(), raw_data_path)
        ...
        for dt in pd.date_range(start=start_date, end=end_date, freq='D'):
            ...
            date_dir = tickdata_remote_dir + dt.strftime('%Y.%m.%d')
            with connection.cd():
